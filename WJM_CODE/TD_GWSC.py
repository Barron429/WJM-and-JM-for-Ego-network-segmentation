        ...
        for i,o in enumerate(options):
            if i == 0:
                if (o[1][0],o[1][1]) not in all_dis_dict:
                    error, representative = GREEDY_WSC(sub_segment[1:,:])
            ...
            if i == 1:
                if (o[0][0],o[0][1]) not in all_dis_dict:
                    error, representative = GREEDY_WSC(sub_segment[0:2,:])
    ...
    if len(sub_segment) > 3:
        ...
        for j in index_sub_segment[1:-2]:
            ...
            if (start,j) not in all_dis_dict:
                error, representative = GREEDY_WSC(sub_segment[0:anchor_index+1,:])
                all_dis_dict[(start,j)] = [error, representative]
            if (j+1,end) not in all_dis_dict:    
                error, representative = GREEDY_WSC(sub_segment[anchor_index+1:])
        ...
        if (start,index_sub_segment[-2]) not in all_dis_dict:
            error, representative = GREEDY_WSC(sub_segment[0:len(index_sub_segment)-1,:])
...
if __name__ == '__main__':
    ...
    start = time.time()
    segmentation,summary_graphs = top_down(sparse_matrix_data,k)
    end = time.time()

...
def Greedy_WSC_d(data,l,r):
    ...
    if r > 1:
        ...
        if new_part_of_data_no_zero.shape[1] > 0:
            ...
            return I-compute_WJD(data,final_summary),final_summary
    ...
    if r == 1:
        ...
        return data.shape[0]- error,summary
        ...
        for i,o in enumerate(options):
            if i == 0:
                if (o[1][0],o[1][1]) not in all_dis_dict:
                    m = sub_segment[1:,:].shape[0]
                    r = math.ceil(math.sqrt(m))
                    error, representative = Greedy_WSC_d(sub_segment[1:,:],l,r)
            ...
            if i == 1:
                if (o[0][0],o[0][1]) not in all_dis_dict:
                    m = sub_segment[0:2,:].shape[0]
                    r = math.ceil(math.sqrt(m))
                    error, representative = Greedy_WSC_d(sub_segment[0:2,:],l,r)
    ...
    if len(sub_segment) > 3:
        ...
        for j in index_sub_segment[1:-2]:
            ...
            if (start,j) not in all_dis_dict:
                m = sub_segment[0:anchor_index+1,:].shape[0]
                r = math.ceil(math.sqrt(m))
                error, representative = Greedy_WSC_d(sub_segment[0:anchor_index+1,:],l,r)
                all_dis_dict[(start,j)] = [error, representative]
            if (j+1,end) not in all_dis_dict:    
                m = sub_segment[anchor_index+1:].shape[0]
                r = math.ceil(math.sqrt(m))
                error, representative = Greedy_WSC_d(sub_segment[anchor_index+1:],l,r)
        ...
        if (start,index_sub_segment[-2]) not in all_dis_dict:
            m = sub_segment[0:len(index_sub_segment)-1,:].shape[0]
            r = math.ceil(math.sqrt(m))
            error, representative = Greedy_WSC_d(sub_segment[0:len(index_sub_segment)-1,:],l,r)

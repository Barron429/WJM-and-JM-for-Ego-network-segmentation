...
def Memoization(sub_segment,all_dis_dict,index_sub_segment):
    ...
    sub_segment_length = len(sub_segment)

    if sub_segment_length > 3:
    ...
    elif sub_segment_length == 1:
        best_index = index_sub_segment[0]
        best_val = 0.5

    elif sub_segment_length == 2:
        best_index = index_sub_segment[0]
        best_val = 1
...
def top_down(sequence,k):
    ...
    flat_reaults = [0] * N
    sorted_index_segment = sorted(index_segment)
    for a,i in enumerate(sorted_index_segment):
        for j in i:
            flat_reaults[j] = a   


    means_index = []
    sorted_cut_point_list = sorted(cut_point_list)
    for i,j in enumerate(sorted_cut_point_list):
        if i == 0:
            means_index.append((0,j))
        else:
            means_index.append((sorted_cut_point_list[i-1]+1,j))
            
    if N > 1:
        means_index.append((sorted_cut_point_list[-1]+1,N-1))  
